...
def smooth_curve_average(points, sample_size):
    ...
    for id in range(sample_size, len(points)):
        ...
        smoothed_points.append(
            previous + (points[id] - points[id - sample_size]) / sample_size)
...
def analisisHR2(hrValues, miliValues, sample_size):
    ...
    smoothed_values = [data for data,
                       i in smooth_curve_simple(hrValues, sample_size)]
    peaks = fp(smoothed_values)[0]
    ...
    plt.plot(smoothed_values)
    plt.scatter(peaks, [smoothed_values[j]
                        for j in peaks], marker='+', c='Red')
    # plt.show()
...
while (1):
    try:
        ...
        # ["HR", "118"], toma el 118, lo convierte a int y lo guarda
        ...
        cursor = cnx.cursor()
        query = f"INSERT INTO Sensor_cardio(id_perfil, medicion_cardio, fecha) values('10', "+HR+", '2020-11-10');"
        cursor.execute(query)
        cnx.commit()
        query = f"INSERT INTO Sensor_oxi(id_perfil, medicion_oxi, fecha) values('10', "+SPO2+", '2020-11-10');"
        cursor.execute(query)
        cnx.commit()
